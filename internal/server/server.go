...
// to a host name) and serves a custom virtual filesystem via sftp.RequestServer.
package server
...
func New(cfg *config.Server, signers []ssh.Signer) (*Server, error) {
	...
	sshCfg := &ssh.ServerConfig{
		PublicKeyCallback: func(_ ssh.ConnMetadata, key ssh.PublicKey) (*ssh.Permissions, error) {
			...
			return &ssh.Permissions{Extensions: map[string]string{"pds-host": host}}, nil
		},
	}
...
func (s *Server) handleConn(nConn net.Conn) {
	...
	host := sconn.Permissions.Extensions["pds-host"]
	...
	for newChan := range chans {
		...
		go s.handleSession(ch, requests, host)
	}
}

func (s *Server) handleSession(ch ssh.Channel, in <-chan *ssh.Request, host string) {
	defer ch.Close()
	for req := range in {
		if req.Type == "subsystem" && isSFTP(req.Payload) {
			_ = req.Reply(true, nil)
			h := newHandlers(s.cfg, host, s.committer)
